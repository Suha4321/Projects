...
## Prerequisites
The coding challenge was performed using python 3.7. The required python libraries to execute the code successfully are os, sys, csv and decimal

## Approach
* The files are large and therefore, instead of fully reading in the entire files, only the required columns and rows were read. e.g
    - Only the products ordered for the first time were extracted from the product_orders.csv on the first read and stored as list -
    - Only the department and the product was read from the product.csv file as a dictionary



*  There are two function to read the files and return the appropriate data structure
    -  'Read_dataset_to_dict' : This function is used to  read the product file and return a dictionary with department as key and list of product as value.
                        The fetch and write operations are faster in the dictionary as compared to list.
                        e.g. {'13': ['19273', '34567']} . Here department 13 has two products associated with it in products.csv.

     - 'Read_dataset_to_list': This function is used to read the product_orders.csv to return a list of products that were ordered for the first time.
                    Iteration though items is faster while using list as compared to a set and hence this data structure.
                        e.g. ['19273'] . Here product  19273 has a reorder value of  which signifies that the product has not been ordered in past.

* The list of first time ordered products is looked up against the {department: list of products} dictionary  and we get the first time order
                    dictionary that associates first time products to their respective dept. e.g. {'13': '19273'}

* now using the {department: list of products} and  {department: list of products ordered for the first time} dictionary we get the list with
    required variables e.g. [13, 2,1,0.50]. Each of these values mean the following -
    - 13 : department
    - 2 : total order per department   
    - 1 : total first time product ordered per department
    - 0.50: ratio of both the above variable

* This list is then sorted and written to a csv file to obtain the required result. We used list.sorted() over sorted(list) as the latter does not create a new list.

* The code is written to minimize the number of 'for' iterations to improve the performance
